...
God.prepare = function prepare (env, cb) {
  ...
  timesLimit(env.instances, 1, function (n, next) {
    ...
    God.injectVariables(env, function inject (err, _env) {
      ...
      // Add flag for job script to be able to detect scheduled run and a run on start of pm2
      //_env.args.push('-pm2-on-start-')

      var __env = Utility.clone(_env)
      console.log(">>>>>>>", _env.args, __env.args)

      if (__env.hasOwnProperty('args')) {
        __env.args.push('-pm2-on-start-')
      }

      return God.executeApp(__env, function (err, clu) {
        if (err) return next(err);
        God.notify('start', clu, true);
